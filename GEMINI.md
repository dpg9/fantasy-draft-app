...
- **Media**: Draft picks are intended to have timer interactions and noise interactions (TODO: Implement sound triggers).

### Core Technologies
- **Frontend**: React 18, Vite, Tailwind CSS (via `@tailwindcss/postcss`), PostCSS.
...
- **Tailwind CSS**: Use utility classes for all styling. Note that the project uses `@tailwindcss/postcss` (v4 compatible).
...
- `client/src/api.js`: Axios-like fetch wrappers for the backend API.
- `client/src/components/DraftBoard.jsx`: Core visualization of the draft grid.
- `start-dev.sh`: Primary entry point for developers.

### TODOs / Roadmap
1. **Audio Interaction**: Implement the "timer with noise interaction" mentioned in requirements.
2. **Team Avatars**: Add the ability to upload or select team avatars.
3. **Draft Settings**: Support "Linear" draft order in addition to "Snake".
4. **Proxmox Deployment**: Create Dockerfile and deployment guide for hosting on Proxmox.
5. **Validation**: Add backend validation for draft picks and team creation.
